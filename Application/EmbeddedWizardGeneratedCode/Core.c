...
const XClass EwApplicationClass = EW_CLASS( ApplicationMainPage );
...
{
  if ( value )
    CoreView__ChangeViewState( _this, 0, CoreViewStateDontClipViews );
  else
    CoreView__ChangeViewState( _this, CoreViewStateDontClipViews, 0 );
}
...
EW_DEFINE_CLASS( CoreWipeTouchHandler, CoreRectView, _.VMT, _.VMT, _.VMT, _.VMT, 
                 _.VMT, _.VMT, "Core::WipeTouchHandler" )
  CoreRectView_initLayoutContext,
  CoreView_GetRoot,
...
void CoreSlideTouchHandler_timerSlot( CoreSlideTouchHandler _this, XObject sender )
{
  ...
  * 0.5f ) * timeY2 ) + ( _this->speedY * timeY )) + _this->startY ));
  parkingTime = (XFloat)_this->RubberBandEffectDuration * 0.001f;
  ...
  if ( _this->parkingX && ( timeX >= parkingTime ))
  {
    ...
    if ( _this->parkingX )
    {
      XFloat f = 1.0f - EwMathPow( 1.0f - ( timeX / parkingTime ), 5.0f );
  ...
  if ( _this->parkingY && ( timeY >= parkingTime ))
  {
    ...
    if ( _this->parkingY )
    {
      XFloat f = 1.0f - EwMathPow( 1.0f - ( timeY / parkingTime ), 5.0f );
...
EW_DEFINE_CLASS( CoreSlideTouchHandler, CoreRectView, timer, timer, OnSlide, state, 
